...
describe "The 'tt' comand line compiler" do
  ...
  context 'when processing a single grammar file' do
    ...
    it 'can compile a grammar file' do
      ...
      system("ruby -S tt #{@test_grammar}").should be true

      File.exists?(@test_ruby).should be true
      File.zero?(@test_ruby).should_not be true
    end
    ...
    it 'can compile a relative pathed grammar file' do
      ...
      system("cd #{@tmpdir}/..; ruby -S tt \"./#{dir}/#{@test_base}.tt\"").should be true
    ...
    it 'can compile an absolute pathed grammar file' do
      ...
      system("ruby -S tt \"#{File.expand_path(@test_grammar)}\"").should be true
    ...
    it 'can compile without explicit file extensions' do
      ...
      system("ruby -S tt #{@test_path}").should be true
    ...
    it 'skips nonexistent grammar file without failing or creating bogus output' do
      ...
      File.exists?("#{@test_base}.rb").should be false
    end
    ...
    it 'can compile to a specified parser source file' do
      ...
      begin
        system("ruby -S tt -o #{pf} #{@test_path}").should be true

        File.exists?(pf).should be true
        File.zero?(pf).should_not be true
      ensure
    ...
    it 'can be forced to overwrite existing file #{@test_path}' do
      pf = "#{@test_path}.rb"
      system("echo some junk >#{pf}").should be true

      File.exists?(pf).should be true
      File.zero?(pf).should_not be true
      orig_file_hash = Digest::SHA1.hexdigest(File.read(pf))

      system("ruby -S tt -f #{@test_path}").should be true
  ...
  context 'when processing multiple grammar files' do
    ...
    it 'can compile them in one invocation' do
      ...
      system("ruby -S tt #{@test_grammars.join(' ')}").should be true
    ...
    it 'can compile them without explicit file extenstions' do
      ...
      system("ruby -S tt #{@test_bases.join(' ')}").should be true
    ...
    it 'can skip nonexistent and invalid extension named grammar files' do
      ...
      system("ruby -S tt not_here bad_ext.ttg #{@test_bases.join(' ')} >/dev/null 2>&1").should be true

      File.exists?('not_here.rb').should_not be true
      File.exists?('bad_ext.rb').should_not be true
    ...
    it 'can not specify an output file' do
      ...
      system("ruby -S tt -o #{pf} #{@test_bases.join(' ')} >/dev/null 2>&1").should be false
      File.exists?(pf).should be false
    end
  end
